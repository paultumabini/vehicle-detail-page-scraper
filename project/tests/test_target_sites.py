...
class TargetSiteLastRunFilterTests(TestCase):
    def test_pending_status(self):
        ...
        self.assertEqual(target_site_last_run(site), 'processing...')

    def test_inactive_status(self):
        site = SimpleNamespace(status='Inactive')
        self.assertEqual(target_site_last_run(site), 'paused')
    ...
    def test_mismatched_dates_returns_running(self):
        site = SimpleNamespace(
            status='Active',
            latest_log_created=datetime(2026, 6, 12, 10, 0, 0),
            latest_scrape_checked=datetime(2026, 6, 13, 10, 0, 0),
        )
        self.assertEqual(target_site_last_run(site), 'running...')


class SiteListViewTests(TestCase):
    def setUp(self):
        self.client = Client()
        self.user = User.objects.create_user(username='site_tester', password='test-pass')
        ...
        Scrape.objects.create(target_site=self.site, stock_number='STK1', last_checked=run_time)
        SpiderLog.objects.create(target_site=self.site, spider_name='test_spider')
    ...
    def test_authenticated_user_sees_target_sites_table(self):
        ...
        self.assertIn('vdp-cell-date', content)
